...
description: Use first when a research project touches a recognisable academic discipline. Picks the strategy template (vocabulary, databases, citation behaviour, date semantics) based on whether the query is sciences, medicine, social sciences, humanities, numerical, or law. Adapted from Suzanne Bell, Librarian's Guide to Online Searching.
metadata:
  portable: true
  compatible_with:
    - claude-code
