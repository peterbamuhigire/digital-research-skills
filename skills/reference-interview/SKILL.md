...
description: Use FIRST when the user states what they want from the engine — most stated queries are not the real question. Encodes Bates's eight clarification questions plus Taylor's "compromised need" detection plus Rowland's premise-statement requirement. The intent-elicitation step that closes the gap between stated and real query.
metadata:
  portable: true
  compatible_with:
    - claude-code
