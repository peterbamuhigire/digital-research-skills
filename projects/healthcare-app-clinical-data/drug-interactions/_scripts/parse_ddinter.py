    ...
    if file.endswith('-data.md'):
        ...
        with open(filepath, 'r', encoding='utf-8') as f:
            ...
            for i, line in enumerate(lines):
                # Start of a table
                if '| atc_code |' in line.lower() or '|---|' in line.lower():
                    in_table = True
                    continue

                # Data row in table (starts with |)
                if in_table and line.strip().startswith('|') and '---' not in line:
                    # Split by pipe
                    cells = [cell.strip() for cell in line.split('|')[1:-1]]
                    if len(cells) >= 9:
                        atc = cells[0]
                        # INN is typically column 8
                        inn = cells[8] if len(cells) > 8 else None

                        if atc and re.match(r'[A-Z]\d{2}[A-Z]{2}\d{2}', atc) and inn and '[GAP' not in inn:
                            if atc not in drug_to_atc:
                                drug_to_atc[atc] = []
                            if inn.lower() not in drug_to_atc[atc]:
                                drug_to_atc[atc].append(inn.lower())

print(f"Drug-to-ATC mapping created: {len(drug_to_atc)} unique ATCs")
...
print(f"Filtered pairs from all CSVs: {len(all_pairs)}")
...
with open('projects/healthcare-app-clinical-data/drug-interactions/research/_ddinter_import_stats.txt', 'w') as f:
    ...
    f.write(f"Total pairs parsed from DDInter: {len(all_pairs)}\n")
